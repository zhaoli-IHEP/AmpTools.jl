using BenchmarkTools, Combinatorics, LinearAlgebra, SymEngine, AmplitudeTools, Dates, Test  
...
@testset "get_degree" begin
  @vars x, y
  poly = 1+2*x+y*3*x^3
  @test AmplitudeTools.get_degree( poly, x ) == 3
end # @testset
...
@testset "make_SP" begin
  ...
  @test AmplitudeTools.make_SP(k1,k2) == SP(k1,k2)
  @test AmplitudeTools.make_SP(k1,-k2) == -SP(k1,k2)
  @test AmplitudeTools.make_SP(-k1,-k2) == SP(k1,k2)
  @test AmplitudeTools.make_SP(k1-k3,k2+k4) == SP(k1, k2) + SP(k1, k4) - SP(k2, k3) - SP(k3, k4)

  @test AmplitudeTools.make_SP( k1, k2 ) == SP(k1, k2)
  @test AmplitudeTools.make_SP( k1, k2+2*k3 ) == SP(k1, k2) + 2*SP(k1, k3)
  test_mom = 2*k1+3*k2+k3+5*k4+6*q1+7*q2
  @test AmplitudeTools.make_SP( test_mom, test_mom ) == 4*SP(k1, k1) + 12*SP(k1, k2) + 4*SP(k1, k3) + 20*SP(k1, k4) + 24*SP(k1, q1) + 28*SP(k1, q2) + 9*SP(k2, k2) + 6*SP(k2, k3) + 30*SP(k2, k4) + 36*SP(k2, q1) + 42*SP(k2, q2) + SP(k3, k3) + 10*SP(k3, k4) + 12*SP(k3, q1) + 14*SP(k3, q2) + 25*SP(k4, k4) + 60*SP(k4, q1) + 70*SP(k4, q2) + 36*SP(q1, q1) + 84*SP(q1, q2) + 49*SP(q2, q2)
end # @testset
...
@testset "get_det_old" begin
  for dim ∈ 1:8
    ...
    diff_result = AmplitudeTools.get_det_old(tmp_mat)
  ...
  for dim ∈ 1:10
    ...
    @test det(tmp_mat) == AmplitudeTools.get_det_old(tmp_mat)
  end
end
...
@testset "get_det" begin
  for dim ∈ 1:8
    ...
    diff_result = AmplitudeTools.get_det(tmp_mat)
    for perm ∈ permutations(1:dim)
      σ = (iseven ∘ parity)(perm) ? 1 : -1
      diff_result -= σ * reduce(*, tmp_mat[ii, perm[ii]] for ii ∈ 1:dim)
    end
  ...
  for dim ∈ 1:10
    tmp_mat = rand(Basic.(0:100), dim, dim)
    @test det(tmp_mat) == AmplitudeTools.get_det(tmp_mat)
  end
end
...
@testset "`get_det_old` vs. `get_det`" begin
  for dim ∈ 1:10
    ...
    @btime AmplitudeTools.get_det_old($tmp_mat)
    println("Bisection `get_det`:")
    @btime AmplitudeTools.get_det($tmp_mat)
    println()
  end
