using BenchmarkTools, Combinatorics, LinearAlgebra, SymEngine, AmpTools, Dates, Test  
...
@testset "get_det_old" begin
  for dim ∈ 1:8
    ...
    diff_result = AmpTools.get_det_old(tmp_mat)
  ...
  for dim ∈ 1:10
    ...
    @test det(tmp_mat) == AmpTools.get_det_old(tmp_mat)
  end
end
...
@testset "get_det" begin
  for dim ∈ 1:8
    ...
    diff_result = AmpTools.get_det(tmp_mat)
    for perm ∈ permutations(1:dim)
      σ = (iseven ∘ parity)(perm) ? 1 : -1
      diff_result -= σ * reduce(*, tmp_mat[ii, perm[ii]] for ii ∈ 1:dim)
    end
  ...
  for dim ∈ 1:10
    tmp_mat = rand(Basic.(0:100), dim, dim)
    @test det(tmp_mat) == AmpTools.get_det(tmp_mat)
  end
end
...
@testset "`get_det_old` vs. `get_det`" begin
  for dim ∈ 1:10
    ...
    @btime AmpTools.get_det_old($tmp_mat)
    println("Bisection `get_det`:")
    @btime AmpTools.get_det($tmp_mat)
    println()
  end
